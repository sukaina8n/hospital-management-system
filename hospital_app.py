...
def page_dashboard():
    ...
    if st.session_state.patients:
        ...
        for p in st.session_state.patients[-3:]:
            ...
            if p.vitals:
                v = p.vitals[-1]
                vital_flag = " ⚠️" if vital_status(v.heart_rate, v.oxygen_level, v.temperature) == "warn" else " ✅"
...
def page_doctors():
    st.markdown('<div class="page-title">👨‍⚕️ Manage Doctors</div>', unsafe_allow_html=True)
...
def page_vitals():
    ...
    with st.form("upload_vitals"):
        ...
        if st.form_submit_button("Upload Vitals"):
            ...
            if status == "warn":
                st.warning(f"⚠️ Vitals uploaded for {patient.name}. Some readings are outside normal range!")
            else:
